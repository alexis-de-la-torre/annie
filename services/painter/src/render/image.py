...
def render_rects(rects, canvas_size=(600, 600)):
    if type(rects) is list and len(rects) == 0:
        return None
    ...
    for rect in rects:
        ...
        if rect.image_ext is not None:
            img_canvas = Image.new('RGBA', canvas_size)

            path = os.path.join(script_dir, '..', '..', 'img_ext', f"{rect.team_id}.png")

            if not os.path.exists(path):
                img_data = requests.get(rect.image_ext).content
...
def render_rects_multi(rects, canvas_size=(600, 600)):
    if type(rects) is list and len(rects) == 0:
        return None
